...
export const RightSidebar: React.FC<RightSidebarProps> = ({ currentTeamId, threadsData, onThreadClick }) => {
    ...
    const notFinishedThreads = pendingThreads.filter(t => !t.waiting_contact);
    const waitingThreads = pendingThreads.filter(t => t.waiting_contact);
    ...
    const renderTaskCard = (t: any) => (
        <div key={t.id} className={`task-card mini-job-card ${t.waiting_contact ? 'is-waiting' : ''}`} style={{ cursor: 'pointer', position: 'relative' }} onClick={() => scrollToThread(t.id)}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginLeft: '10px', flexShrink: 0 }}>
                    <button
                        className={`btn btn-sm btn-status btn-waiting ${t.waiting_contact ? 'active' : ''}`}
                        onClick={(e) => { e.stopPropagation(); handleToggleWaiting(t.id, !!t.waiting_contact); }}
                        title={t.waiting_contact ? '連絡待ちを解除' : '連絡待ちにする'}
                        style={{ width: '28px', height: '28px', padding: 0, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', border: '1px solid rgba(255, 255, 255, 0.4)' }}
                    >
                ...
                <div style={{ paddingBottom: '20px' }}>
                    ...
                    <div id="pending-sidebar-list">
                        {notFinishedThreads.length === 0 ? (
                            ...
                            notFinishedThreads.map(renderTaskCard)
                        )}
                    </div>
                </div>
                ...
                <div className="side-panel-section">
                    ...
                    <div id="waiting-sidebar-list">
                        {waitingThreads.length === 0 ? (
                            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', padding: '10px' }}>連絡待ちのタスクはありません</div>
                        ) : (
                            waitingThreads.map(renderTaskCard)
                        )}
                    </div>
                </div>
