...
- **UI 変更を本番反映するときは `src/data/changelog.ts` の先頭に
  `# YYYY-MM-DD 更新履歴` セクションを必ず追記する（デフォルト運用）**。
  起動時の「新機能のお知らせ」ダイアログ（`WhatsNewModal.tsx`）は changelog の
  最新セクションを自動表示するため、追記するだけで全ユーザーに 1 回表示される。
  ダイアログ側のコード変更は不要。
